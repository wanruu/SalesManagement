...
# 3. 修改order的基本信息（不含invoiceItems），检查partner列表，检查order和refund列表和详情
...
def run():
    ...
    # 3. 修改order的基本信息（不含invoiceItems），检查partner列表，检查order和refund列表和详情
    t_desc = "Update order 1 basic info."
    new_order = generate_order(partner2, "2034-02-01", order_items)
    order_res = {
        "id": 1,
        "type": "salesOrder",
        "number": order_number,
    ...
    t = TestStep.get("/partners", t_desc, 200, [
        { "name": partner1["name"], **empty_partner, "invoiceNum": 0 },
        { "name": partner2["name"], **empty_partner, "invoiceNum": 2 },
    ])
    testcase.add_step(t)
    ...
    t = TestStep.get("/salesRefunds", t_desc, 200, [{
        ...
        **from_dict(refund, ["date", "amount", "prepayment", "payment"]),
        ...
        "order": {
            ...
            "type": "salesOrder",
            "partnerName": partner2["name"],
            **from_dict(new_order, ["date", "amount", "prepayment", "payment"]),
            "orderId": None
        },
        "deliveredItemNum": 1,
        "totalItemNum": 1
