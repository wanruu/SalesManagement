...
# 3. 删除partner1，对应的清单都被删除，检查order、refund、product列表
...
def run():
    ...
    # 3. 删除partner1，对应的清单都被删除，检查order、refund、product列表
    t_desc = "Delete partner 1."
    t = TestStep.delete(f"/partners/{partner1['name']}", t_desc, 204, None)
    testcase.add_step(t)
    ...
    t = TestStep.get("/products", t_desc, 200, [
        ...
        { "id": 2, **product2, "invoiceItemNum": 0 },
        { "id": 3, **product3, "invoiceItemNum": 0 }
    ])
    testcase.add_step(t)
