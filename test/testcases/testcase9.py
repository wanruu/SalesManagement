...
# 3. 修改refund的基本信息（不含invoiceItems），检查partner列表，检查order和refund列表和详情
...
def run():
    ...
    # 3. 修改refund的基本信息（不含invoiceItems），检查partner列表，检查order和refund列表和详情
    t_desc = "Update refund 2 basic info."
    new_refund = generate_refund(order, "2034-02-01", 1, refund_items)
    new_refund["partnerName"] = "000"
    refund_res = {
        "id": 2,
        "type": "salesRefund",
        "number": refund_number,
        "partnerName": partner1["name"],
        **from_dict(new_refund, ["date", "amount", "prepayment", "payment"]),
        "orderId": 1,
        "order": {
            "id": 1,
            "number": order_number,
    ...
    t = TestStep.get("/partners", t_desc, 200, [
        { "name": partner1["name"], **empty_partner, "invoiceNum": 2 }
    ])
    testcase.add_step(t)
